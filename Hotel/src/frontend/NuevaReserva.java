...
public class NuevaReserva {
	...
	public static void main(String[] args) {
		...
		PantallaReservas.setTitle("Hotel Mi Reina C.A - Crear reserva");
		...
		ImagePanel panel = new ImagePanel(new ImageIcon(Toolkit.getDefaultToolkit().getImage(NuevaReserva.class.getResource("/imagenes/BG.jpg"))).getImage());
		...
		btnSalir.setIcon(new ImageIcon(NuevaReserva.class.getResource("/imagenes/LogoutIcon(1).png")));
		...
		JLabel lblCuartos = new JLabel("Registrar reserva de cuarto");
		...
		NuevaReserva.comboboxclientes = new JComboBox();
		...
		PantallaReservas.setIconImage(Toolkit.getDefaultToolkit().getImage(NuevaReserva.class.getResource("/imagenes/HotelIcon.png")));
		...
		fechasalida.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent arg0) {
				 if ("date".equals(arg0.getPropertyName()) && !((JTextField)fechaentrada.getDateEditor().getUiComponent()).getText().equalsIgnoreCase("")) {
					 LocalDateTime from = LocalDateTime.ofInstant(fechaentrada.getDate().toInstant(), ZoneId.systemDefault());
		...
		fechaentrada.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				Date input = fechaentrada.getDate();
				
				
				 if ("date".equals(evt.getPropertyName())) {
					 LocalDate date = input.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
						fechasalida.setMinSelectableDate(java.sql.Date.valueOf( date.plusDays(1) ));
						...
						try {
							...
							op.CargarReservas(tablareservas, date);
							lblReservas.setText("Reservas - " + text);
		...
		fechaentrada.setBounds(175, 292, 139, 28);
		fechaentrada.setMinSelectableDate(java.sql.Date.valueOf( hoy ));
		...
		comboboxhabitaciones.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(comboboxhabitaciones.getSelectedItem().toString().equalsIgnoreCase("Seleccionar")){
					...
					((JTextField)fechaentrada.getDateEditor().getUiComponent()).setText("");
					((JTextField)fechasalida.getDateEditor().getUiComponent()).setText("");
				...
				if(!comboboxhabitaciones.getSelectedItem().toString().equalsIgnoreCase("Seleccionar")){
					...
					try {
					 ...
					 if(!((JTextField)fechaentrada.getDateEditor().getUiComponent()).getText().equalsIgnoreCase("") && !((JTextField)fechasalida.getDateEditor().getUiComponent()).getText().equalsIgnoreCase("")){
			 				LocalDateTime from = LocalDateTime.ofInstant(fechaentrada.getDate().toInstant(), ZoneId.systemDefault());
			 				LocalDateTime to = LocalDateTime.ofInstant(fechasalida.getDate().toInstant(), ZoneId.systemDefault());
		...
		btnaceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(((JTextField)fechaentrada.getDateEditor().getUiComponent()).getText().equalsIgnoreCase("")){
					JOptionPane.showMessageDialog(null, "Error: Debe seleccionar una fecha de entrada!", "Hotel Mi Reina C.A - Error", JOptionPane.WARNING_MESSAGE);
					return;
				}
				...
				BD op = new BD();
				op.Conectar("Hotel");
				LocalDateTime entrada = LocalDateTime.ofInstant(fechaentrada.getDate().toInstant(), ZoneId.systemDefault());
				 LocalDateTime salida = LocalDateTime.ofInstant(fechasalida.getDate().toInstant(), ZoneId.systemDefault());
				try {
					if(op.BuscarReserva(Integer.parseInt(comboboxhabitaciones.getSelectedItem().toString()), entrada, salida) == true){
		...
		btnaceptar.setIcon(new ImageIcon(NuevaReserva.class.getResource("/imagenes/AcceptIcon.png")));
		...
		btnborrar.setIcon(new ImageIcon(NuevaReserva.class.getResource("/imagenes/EraserIcon.png")));
